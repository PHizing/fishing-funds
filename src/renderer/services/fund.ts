...
// 解析新浪基金数据
export function ParseSinaFundData(code: string, rawFu?: string[], rawF?: string[], rawStock?: string[]): Fund.ResponseItem {
  let name = '';
  let dwjz = '';
  let gsz = '';
  let gszzl = '';
  ...
  // 1. 先解析 rawF (最新公布净值数据: 基金名称, 单位净值, 累计净值, 前一日净值, 净值日期...)
  if (rawF && rawF.length > 4) {
    name = rawF[0]?.trim() || '';
    dwjz = rawF[1]?.trim() || '';
    jzrq = rawF[4]?.trim() || '';
  }

  // 2. 解析 rawFu (实时估值数据: 基金名称, 估值时间, 估算净值, 昨日单位净值, 累计净值, 增长额, 估算增长率, 估算日期...)
  if (rawFu && rawFu.length > 7) {
    if (!name) name = rawFu[0]?.trim() || '';
    if (!dwjz) dwjz = rawFu[3]?.trim() || '';
    gsz = rawFu[2]?.trim() || '';
    const rawGszzl = rawFu[6]?.trim();
    if (rawGszzl !== undefined && rawGszzl !== '') {
      gszzl = Number(rawGszzl).toFixed(2);
    }
    const timeStr = rawFu[1]?.trim();
    const dateStr = rawFu[7]?.trim();
    if (dateStr && timeStr) {
      gztime = `${dateStr} ${timeStr.slice(0, 5)}`;
    }
  }

  // 3. 如果是场内 ETF (159, 16, 50, 51, 56, 58, 59 开头)，且估算值不全，使用 rawStock 场内实时行情
  const isEtfOrLof = /^(159|16|50|51|56|58|59)/.test(code);
  if (isEtfOrLof && (!gsz || !gszzl || gszzl === '0.00') && rawStock && rawStock.length > 3) {
    if (!name) name = rawStock[0]?.trim() || '';
    const currentPrice = rawStock[1]?.trim();
    const zdf = rawStock[3]?.trim();
    if (currentPrice && Number(currentPrice) > 0) {
      gsz = currentPrice;
      gszzl = Number(zdf).toFixed(2);
      gztime = new Date().toISOString().slice(0, 10);
    }
  }
...
// 批量从新浪获取基金实时估值
export async function FromSinaBatch(codes: string[]): Promise<Fund.ResponseItem[]> {
  if (!codes || !codes.length) return [];
  try {
    const listParams: string[] = [];
    codes.forEach((c) => {
      listParams.push(`fu_${c}`);
      listParams.push(`f_${c}`);
      if (/^(50|51|56|58)/.test(c)) {
        listParams.push(`s_sh${c}`);
      } else if (/^(159|16)/.test(c)) {
        listParams.push(`s_sz${c}`);
      }
    });

    const { body } = await request(`http://hq.sinajs.cn/list=${listParams.join(',')}`, {
      headers: {
    ...
    if (body instanceof ArrayBuffer) {
      text = new TextDecoder('gbk').decode(body);
    ...
    for (const line of lines) {
      const match = line.match(/var hq_str_([a-zA-Z0-9_]+)="([^"]*)";?/);
      if (match) {
        dataMap[match[1]] = match[2].split(',');
      }
    }

    const results: Fund.ResponseItem[] = [];
    for (const code of codes) {
      const rawFu = dataMap[`fu_${code}`];
      const rawF = dataMap[`f_${code}`];
      const rawStock = dataMap[`s_sh${code}`] || dataMap[`s_sz${code}`];

      if ((!rawFu || rawFu.length < 2) && (!rawF || rawF.length < 2) && (!rawStock || rawStock.length < 2)) {
        const fallback = await FromEastmoney(code);
        if (fallback) {
      ...
      results.push(ParseSinaFundData(code, rawFu, rawF, rawStock));
    }
    return results;
  } catch (error) {
    const fallbackList = await Promise.all(codes.map((c) => FromEastmoney(c)));
    return fallbackList.filter(Boolean) as Fund.ResponseItem[];
  }
}

// 单只从新浪获取
export async function FromSina(code: string): Promise<Fund.ResponseItem | undefined> {
  const list = await FromSinaBatch([code]);
  return list[0];
}

// 天天基金（移动端 API + 新浪实时估值融合）
export async function FromEastmoney(code: string): Promise<Fund.ResponseItem | undefined> {
  try {
    const sinaResult = await FromSina(code);
    if (sinaResult && sinaResult.name && sinaResult.gsz && sinaResult.gsz !== '0.0000') {
      return sinaResult;
    }
    ...
    if (body?.Datas) {
      ...
      return {
        fundcode: code,
        name: d.SHORTNAME || sinaResult?.name || code,
        dwjz: d.DWJZ || sinaResult?.dwjz || '0.0000',
        gsz: d.DWJZ || sinaResult?.gsz || '0.0000',
        gszzl: d.RZDF ? Number(d.RZDF).toFixed(2) : (sinaResult?.gszzl || '0.00'),
        gztime: d.FSRQ || sinaResult?.gztime || '',
        jzrq: d.FSRQ || sinaResult?.jzrq || '',
      };
    }
    return sinaResult;
  } catch (error) {
